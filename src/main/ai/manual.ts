...
const handleAccept = async (id: string): Promise<MsgType | null> => {
    ...
    // 记录自动创建的数据 id，更新状态
    const [msg] = await services.record.updateById(id, { ...record, status: RecordStatus.FINISH });
    if (msg) return msg;
...
const handleIgnore = async (id: string): Promise<MsgType | null> => {
    ...
    // 更新状态
    const [msg] = await services.record.updateById(id, { status: RecordStatus.FINISH });
    if (msg) return msg;
