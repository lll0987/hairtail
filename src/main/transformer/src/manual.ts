...
const RStatus = { SEND: 0, WAITING: 1, ACCEPT: 2, IGNORE: 3, NONE: 4, ERROR: 5 } as const;
...
export const handleCache = async (text: string, window: BrowserWindow): Promise<string | null> => {
    ...
    const status = obj.status === 1 ? RStatus.WAITING : RStatus.NONE;
...
export const handleAccept = async (id: string): Promise<string | null> => {
    ...
    if (m) return m;
    if (status !== RStatus.WAITING) return '当前结果不可执行接受操作';
    ...
    // 记录自动创建的数据 id，更新状态
    const [msg] = await services.record.updateById(id, { ...record, status: RStatus.ACCEPT });
    if (msg) return msg;
...
export const handleIgnore = async (id: string): Promise<string | null> => {
    ...
    if (m) return m;
    if (status !== RStatus.WAITING) return '当前结果不可执行忽略操作';

    // 更新状态
    const [msg] = await services.record.updateById(id, { status: RStatus.IGNORE });
    if (msg) return msg;
